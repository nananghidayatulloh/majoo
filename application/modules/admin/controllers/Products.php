    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $x) {
            ...
            $row[] = $no;
             $row[] = $x->name;
              $row[] = $x->category_name;
               $row[] =  number_format($x->price,2, ",", ".");
                $row[] = $x->stock;
                 $row[] = $x->product_unit;
                    if($x->is_available ==1){
                $row[] = '<span class="float-right badge bg-success">ACTIVE</span>';
            }else{
                $row[] = '<span class="float-right badge bg-danger">INACTIVE</span>';
            }
                   if($x->picture_name){
                $row[] = '<a href="'.base_url('assets/uploads/products/'.$x->picture_name).'" target="_blank"><img src="'.base_url('assets/uploads/products/'.$x->picture_name).'" class="img-responsive" width="70" height="50"/></a>';
            }else{
                $row[] = '(No photo)';
            }
                     $row[] = '<a href="'.base_url('admin/products/view/'. $x->id).'" class="btn btn-info btn-sm"><i class="fa fa-eye"></i></a>
            <a href="'.base_url('admin/products/edit/'. $x->id).'" class="btn btn-success btn-sm"><i class="fa fa-edit"></i></a>';
